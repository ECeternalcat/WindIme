...
### 符号 / 定型文面板
全屏面板，顶部五个页签：**中 / EN / 颜 / α / 定型文**（◀▶ 切换页签）。中文/英文标点为 4 列网格；颜文字与希腊字母提供特殊符号集；定型文页以单列变宽行列出用户预设的常用短语。选中任意条目即上屏。

### MENU 快捷菜单
京瓷机型检测到 Softkey Guide 时，校准向导自动将左软键绑定为快捷菜单入口（详见下文「京瓷 Softkey Guide 联动」）。其他机型若系统把左软键标记为 **MENU**，也可通过按键校准把该场景实际发送的 KeyCode/ScanCode 绑定为「快捷菜单」。标准 Android `KEYCODE_MENU` 仍可直接打开：
...
### 按键校准与多配置管理（0-Touch）
仅校准**真正因机型而异的功能键**：中英切换、符号表、快捷菜单、退格、回车、关闭输入法（除前三项外均可跳过）。提供只读系统默认映射和 4 个可命名的用户配置，可查看、切换、覆盖校准及删除。
- 数字 0-9、方向键、确认键等 Android 标准键**无需绑定**（由内置兜底表硬性保障）。
- `*` / `#` 可自由绑定（日系机常作符号/回车键）。
- **回车（ENTER）**：纯换行，用于格式文本；建议绑 `*`（701KC 丝印即回车）。与 OK 区分——OK 触发编辑动作（发送/完成）。
- **关闭输入法（DISMISS）**：收起 IME / 退出全屏，文本保留；建议绑拨号/SEND 键或某功能键（BACK 已作退格、OK 已作确认时用它来关闭）。
- **京瓷机型**：检测到 Softkey Guide 时，向导自动跳过「快捷菜单」和「关闭输入法」（由京瓷中软键完成协议接管），改为校准**左软键（→快捷菜单）**和**右软键（→符号面板）**。
- 校准时 **OK / ENTER** 或 **右方向键**跳过当前步骤，左方向键返回上一步；标准方向键和确认键不会被误绑定。

### 京瓷 Softkey Guide 联动（机型自适应）
京瓷 KYF / NP701KC 等机型内置厂商 `NfpSoftkeyGuide` 框架，WindIme 通过反射自动检测并联动底部软键栏标签：
- **左软键 → 快捷菜单**（标签「菜单」），**右软键 → 符号面板**（标签「符号」）——校准绑定实体左/右软键后自动生效。
- **中软键 → 完成**：在 SoftBank 邮件等应用中，composing 为空时中软键标签显示「完成」，按下即触发宿主完成动作（通过 `Finish_IME` 私有命令实现，等效系统原生输入法的「完成」按钮）。
- 非 Kyocera 机型不显示软键校准项，标签联动也不生效，不影响正常使用。

### 设置页（D-Pad 全程可达）
主菜单七分类：输入设定 / 输入法与按键 / Rime 词库与部署 / 用户词典 / 定型文 / 重置 / 关于 WindIme。每页顶部带 WindIme 品牌色标题栏，列表焦点为浅蓝底 + 深蓝细框（系统设置风格）。
- **输入设定**：设置默认输入法（启用 + 切换两步引导）、模式循环、按键反馈（震动/声音/无）、首字母大写、Multi-tap 间隔、全屏输入模式
- **Rime 词库与部署**：查看部署状态/词库版本/数据体积；重新部署、清理编译缓存、清除用户学习（异步排队，下次服务启动执行）
- **用户词典**：自造词（拼音→汉字）CRUD，自动并入候选；支持导出/导入（写入应用专属外部目录，可用 ADB/MTP 取放）
- **定型文 / 常用短语**：邮箱 / 问候 / 个人信息等快捷短语 CRUD；编辑可保存/复制/删除互不覆盖；支持导出/导入
- **重置**：切换默认按键映射 / 清除 Rime 学习 / 清空用户词典 / 清空定型文 / 清除全部设置（各档独立确认）
- **关于**：版本信息、开源许可证（rime-ice GPL v3、librime BSD 等）
...
### 全屏输入模式（可选）
设置 → 输入设定 → 全屏输入模式。开启后聚焦输入框时，输入法以全屏白底覆盖屏幕：顶部白色文本区镜像宿主编辑框内容（带 `│` 光标位置标记），底部是和平时完全一样的候选栏；遮挡背景杂乱信息，适合小屏。文本仍**增量上屏**（安全），切换该开关后下一个输入框即生效。
- **如何关闭/发送**：因日系机 BACK 常作退格、OK 常作回车，可校准一颗**关闭输入法（DISMISS）**键来收起全屏/输入法（文本保留，回到宿主即可用其发送按钮）；也可校准**回车（ENTER）**键（如 `*`）用于格式换行。
- 无 composing 时 ◀▶ 移动宿主光标（便于改前面的字）；有 composing 时 ◀▶ 仍导航候选。
...
3. 点任意文本框：底部出现模式条 `[中][拼][En][Abc][123][符]`，◀▶ 选模式。
4. 中文 T9 示例（打「你好」）：
...
| `compat/` | `SoftkeyGuideHelper`（反射式厂商 `NfpSoftkeyGuide` 适配，京瓷软键栏标签联动） |
...
当前 `versionCode 21 / 0.5.4`。中文 T9（三层）、英文 T9、Multi-tap、模式条、符号/定型文面板、设置页、用户词典、全屏输入、京瓷 Softkey Guide 联动均已可用，仍持续迭代中。详见 `Document/` 下的设计与路线图文档。
